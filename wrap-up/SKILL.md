...
This skill file must live under the **primary working directory's** `.claude/skills/` to be invocable as `/wrap-up` — Claude Code does not scan `.claude/skills/` in additional working directories for slash-command registration. If you use this skill from multiple repositories or working-directory setups, keep a copy in each one's own `.claude/skills/wrap-up/SKILL.md` and update all copies when this file changes, the same convention `prepare-compact` and `resume` already use. A newly added or edited skill file also typically requires a fresh session before Claude Code picks it up.
